...
public:
	...
	void DoGeneration() {
		...
		lastDiff = SumPlants() - lastSum;
	}

	int64_t SumPlants() const noexcept {
		int64_t ret = 0;
	...
	int64_t LastDiff() const noexcept {
		return lastDiff;
	}
};
...
int main(int argc, char* argv[]) {
	...
	Greenhouse gh{file};
	uint64_t total = 50'000'000'000;
	uint64_t rolls = 1000;
	for (auto i = rolls; i > 0; --i) {
		gh.DoGeneration();
	}
	auto sum = gh.SumPlants() + ((total - rolls) * gh.LastDiff());
	std::cout << "Sum: " << sum << '\n';
}
