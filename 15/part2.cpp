...
struct ElfKilled : std::runtime_error {
...
private:
    ...
    void Attack(Combatant& target, IMap& map) {
        target.health -= power;
        if (target.health <= 0) {
            if (target.Type() == UnitType::Elf)
                throw ElfKilled{"killed"};
            map.Remove(target);
        }
    }
...
protected:
    Combatant(int power, const Tile& curTile) : curTile{curTile}, power{power} {}
...
public:
    Elf(const int power, const Tile& curTile) : Combatant{power, curTile} {}
...
class Map : public IMap {
    ...
    decltype(map) BuildMap(std::istream& in, const int elfPower) {
        decltype(map) ret;
        auto x = 0;
        auto y = 0;
        while (in) {
            switch (in.get()) {
                ...
                case 'E': {
                    ...
                    auto elfPtr = std::make_unique<Elf>(elfPower, e.first->first);
                    auto& elf = *elfPtr;
...
public:
    Map(std::istream& in, const int elfPower) : map{BuildMap(in, elfPower)} {}
